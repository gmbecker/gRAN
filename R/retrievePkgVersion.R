...
##' @export
locatePkgVersion = function(name, version, manifest, repo = NULL,
...
findPkgVersionInManifest = function(name, version, manifest, dir, repo) {
    ...
    pkgdir = makePkgDir(name, source = source, path = dir)
...
findBiocSVNRev = function(name, version, destpath, repo, biocVers="trunk")
{
...
##        addl_dir = paste0("BioC_", biocVers)
    ...
    findSVNRev(name, version, svn_repo = repoloc, destpath, repo)
