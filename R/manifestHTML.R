...
manifestHTML = function(repo)
    {
        ...
        addChildren(summary, htmlParse(hwrite(as.matrix(table(repo@manifest$lastAttemptStatus)))))
        tmpman = repo@manifest[,c("name", "lastAttemptVersion",
        ...
        addChildren(body, htmlParse(hwrite(tmpman)))
