...
mechanism for package distribution. \gran provides three major areas of functionality:
\begin{enumerate}
...
The \Rfunction{ManifestRow} convenience function is provided to assist in the programmatic construction of a GRAN manifest.
...
We use a toy sessionInfo example, because while functional, some aspects of the package search process are too expensive to be run in a vignette, particularly for old Bioconductor packages.
